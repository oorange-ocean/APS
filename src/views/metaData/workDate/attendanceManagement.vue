    ...
    <div class="container">
        ...
        <div class="main" ref="tableContainer">
            <div class="common" ref="commonPlan">
                <common-plan class="plan" :columnNames="attendanceManagement.attendance.column"
                    :viewColumn="attendanceManagement.attendance.viewColumn" :currentViewId="currentViewId"
                    :currentViewName="currentViewName" :apiUrl="'/masterData/searchLike'" :currentTableId="67"
                    :currentOrder="currentOrder" @lookView="lookView" @searchView="searchView"
                    @getCurrentOption="getCurrentOption" />
            </div>
            <el-table ref="myTable" :data="attendanceManagement.attendance.data" border
                ...
                <el-table-column v-for="column in dynamicColumns" :key="column.prop" :prop="column.prop"
                    ...
                    :sort-orders="column.sortOrders" :minWidth="column.minWidth">
            ...
            <div class="bottom" :style="{
                ...
                <Pagination :total="attendanceManagement.attendance.pages" @change-page="handlePages"
                    @update-size="handleSizeChange" :totalRows="attendanceManagement.attendance.total"
...
import { columnConfig } from '../../../columnConfig/metaData/workDate/attendanceManagement'
...
const dynamicColumns = computed(() => {
    if (currentViewId.value === '-1') {
        ...
        return columnConfig
    } else if (!attendanceManagement.attendance.viewColumn) {
        return []
    }

    return attendanceManagement.attendance.viewColumn
        .map(viewCol => {
...
function lookView(viewId, viewName) {
    ...
    attendanceManagement
        ...
        .then((res) => {
            ...
            if (viewId == '-1') {
                // 当 viewId 为 -1 时，使用所有列
                attendanceManagement.attendance.viewColumn = columnConfig.map(col => ({
                    voColName: col.prop,
