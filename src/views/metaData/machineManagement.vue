    ...
    <div class="container">
        ...
        <div class="main" ref="tableContainer">
            <div class="common" ref="commonPlan">
                <common-plan class="plan" :columnNames="useMachine.machine.column"
                    :viewColumn="useMachine.machine.viewColumn" :currentViewId="currentViewId"
                    :currentViewName="currentViewName" :apiUrl="'/masterData/searchLike'" :currentTableId="68"
                    :currentOrder="currentOrder" @lookView="lookView" @searchView="searchView"
                    @getCurrentOption="getCurrentOption" />
            </div>
            <el-table ref="myTable" :data="useMachine.machine.data" border :row-class-name="tableRowClassName"
                ...
                <el-table-column v-for="column in dynamicColumns" :key="column.prop" :prop="column.prop"
                    ...
                    <template v-slot:default="scope">
                        <span v-if="scope.row.editable">
                            ...
                            <div v-if="column.prop === 'unavailableDates'">
                                <div v-for="date in scope.row.unavailableDates" :key="date">
                                    {{ date }}
                                </div>
                                <el-button @click="adjustTime(scope.row)">调整时间</el-button>
                            </div>
                            <el-input v-else-if="column.inputType === 'el-input'" v-model="scope.row[column.prop]"
                                @keyup.enter="saveRow(scope.row)" />
                            <el-autocomplete
                                v-else-if="column.inputType === 'el-autocomplete' && column.prop === 'fProductFamily'"
                                v-model="scope.row[column.prop]" :fetch-suggestions="querySearchProductFamily" clearable
                                class="inline-input w-50" placeholder="Please Input"
                                @select="(item) => handleSelect(item, scope.row)" />
                            <el-autocomplete
                                v-else-if="column.inputType === 'el-autocomplete' && column.prop === 'fProcess'"
                        ...
                        <span v-else>
                            ...
                            <span v-else-if="column.format">{{ formatNumber(scope.row[column.prop]) }}</span>
            ...
            <div class="bottom" :style="{
                ...
                <Pagination :total="useMachine.machine.pages" @change-page="handlePages" @update-size="handleSizeChange"
                    :totalRows="useMachine.machine.total" :currentPage="currentPage" />
            </div>
        </div>
            ...
            <el-dialog v-model="dialogTableVisible" title="调整日期">
                <div>
                    ...
                    <div v-for="(date, index) in adjustDate" :key="index" class="date-row">
                        <el-date-picker v-model="adjustDate[index]" type="datetimerange" start-placeholder="开始时间"
                            end-placeholder="结束时间" :default-time="defaultTime1" value-format="YYYY-MM-DD HH:mm:ss"
...
import { columnConfig } from '../../columnConfig/metaData/machineManagement'
...
const dynamicColumns = computed(() => {
    if (currentViewId.value === '-1') {
        ...
        return columnConfig
    } else if (!useMachine.machine.viewColumn) {
        return []
    }

    return useMachine.machine.viewColumn
        .map(viewCol => {
...
function lookView(viewId, viewName) {
    ...
    useMachine
        ...
        .then((res) => {
            ...
            if (viewId == '-1') {
                // 当 viewId 为 -1 时，使用所有列
                useMachine.machine.viewColumn = columnConfig.map(col => ({
                    voColName: col.prop,
