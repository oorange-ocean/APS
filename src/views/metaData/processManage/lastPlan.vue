    ...
    <div class="container">
        ...
        <div class="main" ref="tableContainer">
            <div class="common" ref="commonPlan">
                <common-plan class="plan" :columnNames="process.lastPlanList.column"
                    :viewColumn="process.lastPlanList.viewColumn" :currentViewId="currentViewId"
                    :currentViewName="currentViewName" :apiUrl="'/masterData/searchLike'" :currentTableId="63"
                    :currentOrder="currentOrder" @lookView="lookView" @searchView="searchView"
                    @getCurrentOption="getCurrentOption" />
            </div>
            <el-table ref="myTable" :data="process.lastPlanList.data" border :row-class-name="tableRowClassName"
            ...
            <div class="bottom" :style="{
                ...
                <Pagination :total="process.lastPlanList.totalPages" @change-page="handlePages"
                    @update-size="handleSizeChange" :totalRows="process.lastPlanList.total"
...
import { columnConfig } from '@/columnConfig/metaData/processManage/lastPlan'
...
const dynamicColumns = computed(() => {
    if (currentViewId.value === '-1') {
        ...
        return columnConfig
    } else if (!process.lastPlanList.viewColumn) {
        return []
    }

    return process.lastPlanList.viewColumn
        .map(viewCol => {
...
function lookView(viewId, viewName) {
    ...
    process
        ...
        .then((res) => {
            ...
            if (viewId == '-1') {
                // 当 viewId 为 -1 时，使用所有列
                process.lastPlanList.viewColumn = columnConfig.map(col => ({
                    voColName: col.prop,
