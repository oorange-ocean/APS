<template>
    <div class="container">
        <div class="head">
            ...
            <button @click="deleteSelectedRows" v-if="userMenu.hasAccessToButton(2, 'delete')">
                <span>删除</span>
            </button>
        ...
        <div class="main" ref="tableContainer">
            <div class="common" ref="commonPlan">
                <common-plan class="plan" :columnNames="process.capacity.column"
                    :viewColumn="process.capacity.viewColumn" :currentViewId="currentViewId"
                    :currentViewName="currentViewName" :apiUrl="'/masterData/searchLike'" :currentTableId="61"
                    :currentOrder="currentOrder" @lookView="lookView" @searchView="searchView"
                    @getCurrentOption="getCurrentOption" />
            </div>
            <el-table ref="myTable" :data="process.capacity.data" border :row-class-name="tableRowClassName"
                ...
                <el-table-column v-for="column in dynamicColumns" :key="column.prop" :prop="column.prop"
                    ...
                    <template v-slot:default="scope">
                        <span v-if="scope.row.editable">
                            ...
                            <el-autocomplete
                                v-else-if="column.inputType === 'el-autocomplete' && column.prop === 'productFamily'"
                            ...
                            <el-autocomplete
                                v-else-if="column.inputType === 'el-autocomplete' && column.prop === 'processName'"
                                v-model="scope.row[column.prop]" :fetch-suggestions="querySearch" clearable
                                class="inline-input w-50" placeholder="Please Input" @select="handleSelect" />
                            <el-select v-else-if="column.inputType === 'el-select' && column.prop === 'packagingMethod'"
                                v-model="scope.row[column.prop]" clearable class="m-2" placeholder="Select"
                                @click="handlePackage">
            ...
            <div class="bottom" :style="{
                ...
                <Pagination :total="process.capacity.totalPages" @change-page="handlePages"
                    @update-size="handleSizeChange" :totalRows="process.capacity.total" :currentPage="currentPage" />
            </div>
        </div>
        <div class="capacity">
...
import { columnConfig } from '../../../columnConfig/metaData/processManage/processCapacity'
...
const dynamicColumns = computed(() => {
    if (currentViewId.value === '-1') {
        ...
        return columnConfig
    } else if (!process.capacity.viewColumn) {
        return []
    }

    return process.capacity.viewColumn
        .map(viewCol => {
...
function lookView(viewId, viewName) {
    ...
    process
        ...
        .then((res) => {
            ...
            if (viewId == '-1') {
                // 当 viewId 为 -1 时，使用所有列
                process.capacity.viewColumn = columnConfig.map(col => ({
                    voColName: col.prop,
