...
onMounted(() => {
    //检查当前url携带的参数中是否有code
    const urlParams = new URLSearchParams(window.location.search);
    const code = urlParams.get('code');
    //如果有code，说明是飞书回调，则调用登录接口，并根据返回结果调用action的飞书登录方法
    if (code) {
        console.log(code)
