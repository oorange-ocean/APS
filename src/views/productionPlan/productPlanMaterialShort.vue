    ...
    <div class="container productSM">
        ...
        <div class="main" ref="tableContainer">
            <div class="common" ref="commonPlan">
                <common-plan class="plan" :columnNames="production.productPlanMaterialShort.column"
                    :viewColumn="production.productPlanMaterialShort.viewColumn" :currentViewId="currentViewId"
                    :currentViewName="currentViewName" :apiUrl="'/scResult/searchLike'" :currentOrder="currentOrder"
                    @lookView="lookView" @searchView="searchView" :currentTableId="44"
                    @getCurrentOption="getCurrentOption" />
            </div>
            <el-table ref="myTable" :data="production.productPlanMaterialShort.data" border
            ...
            <div class="bottom" :style="{
                ...
                <Pagination :total="production.productPlanMaterialShort.pages" @change-page="handlePages"
                    @update-size="handleSizeChange" :totalRows="production.productPlanMaterialShort.total"
...
import { columnConfig } from './productPlanMaterialShortColumnConfig'
...
const dynamicColumns = computed(() => {
    ...
    return production.productPlanMaterialShort.viewColumn
        .map(viewCol => {
...
function lookView(viewId, viewName) {
    ...
    production
        ...
        .then((res) => {
            ...
            viewColumn = production.productPlanMaterialShort.viewColumn
            // console.log(viewColumn,'viewColumn')
            if (viewId == '-1') {
                production.productPlanMaterialShort.viewColumn = columnConfig.map(col => ({
                    voColName: col.prop,
...
function refresh() {
    ...
    production.getViews(tableId.value).then((res) => {
        currentViewId.value = production.productPlanMaterialShort.defaultViewId
        currentViewName.value = production.productPlanMaterialShort.defaultViewName
        // 获取所有的列
        production.getCols(tableId.value).then((res) => {
            // 获取到列名和视图列后再赋值给column和viewColumn
            column = production.productPlanMaterialShort.column
            viewColumn = production.productPlanMaterialShort.viewColumn
            // 如果是“全部”就给plan赋值
            if (currentViewId.value == -1) {
            ...
            production
                ...
                .then((res) => {
                    ...
                    renderSortIcon({ prop: 'ftakeId', order: 'ascending' })
                })
                .catch((error) => { })
        })
    })
