    ...
    <div class="container">
        ...
        <div class="main" ref="tableContainer">
            <div class="common" ref="commonPlan">
                <common-plan class="plan" :columnNames="production.sFMaterialShort.column"
                    :viewColumn="production.sFMaterialShort.viewColumn" :currentViewId="currentViewId"
                    :currentViewName="currentViewName" :apiUrl="'/scResult/searchLike'" :currentOrder="currentOrder"
                    @lookView="lookView" @searchView="searchView" :currentTableId="46"
                    @getCurrentOption="getCurrentOption" />
            </div>
            <el-table ref="myTable" :data="production.sFMaterialShort.data" border
            ...
            <div class="bottom" :style="{ width: userMenu.isCollapse ? 'calc(100vw - 50px)' : 'calc(100vw - 250px)' }">
                <Pagination :total="production.sFMaterialShort.pages" @change-page="handlePages"
                    @update-size="handleSizeChange" :totalRows="production.sFMaterialShort.total"
...
import { columnConfig } from './sFMaterialShortColumnConfig';
...
const dynamicColumns = computed(() => {
    if (currentViewId.value === '-1') {
        ...
        return columnConfig
    } else if (!production.sFMaterialShort.viewColumn) {
        return []
    }

    return production.sFMaterialShort.viewColumn
        .map(viewCol => {
...
function lookView(viewId, viewName) {
    ...
    production
        ...
        .then((res) => {
            ...
            if (viewId == '-1') {
                production.sFMaterialShort.viewColumn = columnConfig.map(col => ({
                    voColName: col.prop,
