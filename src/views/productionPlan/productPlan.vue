  ...
  <div class="container">
    <div class="main" ref="tableContainer">
      <div class="common" ref="commonPlan">
          <common-plan 
          class="plan"
          :columnNames="production.productPlan.column"
      ...
      <el-table
        :data="production.productPlan.data"
        border
      ...
      >
        ...
        <el-table-column prop="fmaterialCode" label="物料编号" 
        width="185"
        ...
        <el-table-column prop="fmaterialName" label="物料名称" width="290" v-if="plan.fmaterialName" sortable="custom" :sort-orders="['ascending','descending']">
          <template v-slot:header="{ column }">
            <div>
              {{ column.label }}
              <span v-html="renderSortIcon(column)"></span>
        ...
        <el-table-column prop="ftotalQuantity" label="总数量" width="100" v-if="plan.ftotalQuantity" sortable="custom" :sort-orders="['ascending','descending']">
          <template v-slot:header="{ column }">
            <div>
              {{ column.label }}
              <span v-html="renderSortIcon(column)"></span>
        ...
        <el-table-column prop="fcompletedQuantity" label="完成数量" width="150" v-if="plan.fcompletedQuantity" sortable="custom" :sort-orders="['ascending','descending']">
          <template v-slot:header="{ column }">
            <div>
              {{ column.label }}
              <span v-html="renderSortIcon(column)"></span>
        ...
        <el-table-column
          prop="factualStartTime"
          label="实际开始时间"
          width="180"
          v-if="plan.factualStartTime"
          sortable="custom"
          :sort-orders="['ascending','descending']"
        >
        <template v-slot:header="{ column }">
        ...
        <el-table-column
          prop="factualCompletionTime"
          label="实际完成时间"
          width="180"
          v-if="plan.factualCompletionTime"
          sortable="custom"
          :sort-orders="['ascending','descending']"
        >
        <template v-slot:header="{ column }">
        ...
        <el-table-column
          prop="fonTimeCompletion"
          label="是否按时完成"
          width="140"
          v-if="plan.fonTimeCompletion"
          sortable="custom"
          :sort-orders="['ascending','descending']"
        >
        <template v-slot:header="{ column }">
        ...
        <el-table-column
          prop="funfinishedReason"
          label="未完成原因"
          width="400"
          v-if="plan.funfinishedReason"
          sortable="custom"
          :sort-orders="['ascending','descending']"
        >
        <template v-slot:header="{ column }">
...
function refresh() {
  ...
  production.getViews(tableId.value).then(res => {
    currentViewId.value = production.productPlan.defaultViewId
    ...
    production.getCols(tableId.value).then(res => {
      ...
      column = production.productPlan.column
      viewColumn = production.productPlan.viewColumn
      // 如果是“全部”就给plan赋值
      if (currentViewId.value == -1) {
        plan.value = column.reduce((acc, item) => {
      ...
      // 获取拥有的数据和所拥有的列
      production
      .getProductionFiltrate({viewId:currentViewId.value},currentPage.value, currentSize.value)
      .then((res) => {
        if (res.code == 201) {
          ElMessageBox.alert(res.message, '提示', {
            confirmButtonText: '好的'
