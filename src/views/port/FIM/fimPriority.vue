    ...
    <div class="container">
        ...
        <div class="main" ref="tableContainer">
            <div class="common" ref="commonPlan">
                <common-plan class="plan" :columnNames="useFimRequest.fimPriority.column"
                    :viewColumn="useFimRequest.fimPriority.viewColumn" :currentViewId="currentViewId"
                    :currentViewName="currentViewName" :apiUrl="'/scResult/searchLike'" :currentTableId="48"
                    :currentOrder="currentOrder" @lookView="lookView" @searchView="searchView"
                    @getCurrentOption="getCurrentOption" />
            </div>
            <el-table ref="myTable" :data="useFimRequest.fimPriority.data" border :row-class-name="tableRowClassName"
            ...
            <div class="bottom" :style="{
                ...
                <Pagination :total="useFimRequest.fimPriority.pages" @change-page="handlePages"
                    @update-size="handleSizeChange" :totalRows="useFimRequest.fimPriority.total"
...
import { downloadTable } from '@/api/productionPlan'
...
import { columnConfig } from '@/columnConfig/FIM/fimPriority'
...
const dynamicColumns = computed(() => {
    if (currentViewId.value === '-1') {
        ...
        return columnConfig
    } else if (!useFimRequest.fimPriority.viewColumn) {
        return []
    }

    return useFimRequest.fimPriority.viewColumn
        .map(viewCol => {
...
function lookView(viewId, viewName) {
    ...
    useFimRequest
        ...
        .then((res) => {
            ...
            viewColumn = useFimRequest.fimPriority.viewColumn
            // console.log(viewColumn,'viewColumn')
            if (viewId == "-1") {
                // 当 viewId 为 -1 时，使用所有列
                useFimRequest.fimPriority.viewColumn = columnConfig.map(col => ({
                    voColName: col.prop,
...
function refresh() {
    ...
    useFimRequest.getViews(tableId.value).then(res => {
        ...
        useFimRequest.getCols(tableId.value).then(res => {
            ...
            column = useFimRequest.fimPriority.column
            viewColumn = useFimRequest.fimPriority.viewColumn
            // 如果是“全部”就给plan赋值
            if (currentViewId.value == -1) {
            ...
            // 获取拥有的数据和所拥有的列
            useFimRequest
                .getFimPriorityFiltrate({ viewId: currentViewId.value }, currentPage.value, currentSize.value)
                .then((res) => {
                    if (res.code == 201) {
                        ElMessageBox.alert(res.message, '提示', {
                            confirmButtonText: '好的'
