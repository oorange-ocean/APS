  ...
  <div class="container">
    ...
    <div class="main" ref="tableContainer">
      ...
      >
        ...
        <el-table-column prop="expectedDeliveryDate" label="期望发货日期" width="200">
          <template #default="{ row }">
            <template v-if="row.editable">
              <el-input v-model="row.expectedDeliveryDate" @keyup.enter="saveRow(row)" />
            </template>
            <template v-else>
        ...
        <el-table-column prop="documentType" label="单据类型" width="200">
          <template #default="{ row }">
            <template v-if="row.editable">
              <el-input
...
function handleSizeChange(newSize) {
  ...
  useFimRequest.getFimRequestPage(currentPage.value, currentSize.value)
    .then((res) => {
...
function handlePages(page) {
  ...
  useFimRequest.getFimRequestPage(page, currentSize.value)
    .then((res) => {
      const scrollContainer = tableContainer.value.querySelector(
        '.el-scrollbar__wrap'
      )
...
function deleteSelectedRows() {
  ...
  if (list.length == 0) {
    ...
    ElMessageBox.confirm('确定删除这些工序吗？', '提示', {
      ...
      .then(() => {
        useFimRequest.removeFimRequest(list)
          .then((res) => {
            console.log('删除产能成功')
            ElMessage({
              type: 'success',
