    ...
    <div class="container">
        ...
        <div class="main" ref="tableContainer">
            <div class="common" ref="commonPlan">
                <common-plan class="plan" :columnNames="useFimRequest.fimRequest.column"
                    :viewColumn="useFimRequest.fimRequest.viewColumn" :currentViewId="currentViewId"
                    :currentViewName="currentViewName" :apiUrl="'/interface/searchLike'" :currentTableId="35"
                    :currentOrder="currentOrder" @lookView="lookView" @searchView="searchView"
                    @getCurrentOption="getCurrentOption" />
            </div>
            <el-table ref="myTable" :data="useFimRequest.fimRequest.data" border :row-class-name="tableRowClassName"
            ...
            <div class="bottom" :style="{
                ...
                <Pagination :total="useFimRequest.fimRequest.pages" @change-page="handlePages"
                    @update-size="handleSizeChange" :totalRows="useFimRequest.fimRequest.total"
...
import { columnConfig } from '@/columnConfig/FIM/fimRequest'
...
const dynamicColumns = computed(() => {
    if (currentViewId.value === '-1') {
        ...
        return columnConfig
    } else if (!useFimRequest.fimRequest.viewColumn) {
        return []
    }

    return useFimRequest.fimRequest.viewColumn
        .map(viewCol => {
...
function lookView(viewId, viewName) {
    ...
    useFimRequest
        ...
        .then((res) => {
            ...
            if (viewId == '-1') {
                // 当 viewId 为 -1 时，使用所有列
                useFimRequest.fimRequest.viewColumn = columnConfig.map(col => ({
                    voColName: col.prop,
