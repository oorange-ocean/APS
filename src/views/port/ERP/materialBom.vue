    ...
    <div class="container">
        ...
        <div class="main" ref="tableContainer">
            <div class="common" ref="commonPlan">
                <common-plan class="plan" :columnNames="ImmediateInventory.materialBom.column"
                    :viewColumn="ImmediateInventory.materialBom.viewColumn" :currentViewId="currentViewId"
                    :currentViewName="currentViewName" :apiUrl="'/interface/searchLike'" :currentTableId="10"
                    :currentOrder="currentOrder" @lookView="lookView" @searchView="searchView"
                    @getCurrentOption="getCurrentOption" />
            </div>
            <el-table ref="myTable" :data="ImmediateInventory.materialBom.data" border
            ...
            <div class="bottom" :style="{
                ...
                <Pagination :total="ImmediateInventory.materialBom.pages" @change-page="handlePages"
                    @update-size="handleSizeChange" :totalRows="ImmediateInventory.materialBom.total"
...
import { columnConfig } from '@/columnConfig/ERP/materialBom'
...
const dynamicColumns = computed(() => {
    if (currentViewId.value === '-1') {
        ...
        return columnConfig
    } else if (!ImmediateInventory.materialBom.viewColumn) {
        return []
    }

    return ImmediateInventory.materialBom.viewColumn
        .map(viewCol => {
...
function lookView(viewId, viewName) {
    ...
    ImmediateInventory.getPageFiltrate(
        ...
        .then((res) => {
            ...
            if (viewId == '-1') {
                // 当 viewId 为 -1 时，使用所有列
                ImmediateInventory.materialBom.viewColumn = columnConfig.map(col => ({
                    voColName: col.prop,
