  ...
  <div class="container">
    ...
    <div class="main" ref="tableContainer">
      ...
      >
        ...
        <el-table-column prop="materialId" label="物料编号" width="180">
          <template #default="{ row }">
            <template v-if="row.editable">
              <el-input
        ...
        <el-table-column prop="remainReceiveQty" label="剩余收料数量" width="180">
          <template #default="{ row }">
            <template v-if="row.editable">
              <el-input v-model="row.remainReceiveQty" @keyup.enter="saveRow(row)" />
            </template>
            <template v-else>
              {{ formatNumber(row.remainReceiveQty) }}
            </template>
          </template>
        </el-table-column>
    ...
    ImmediateInventory.update(
      {
        ...
        "remainReceiveQty": row.remainReceiveQty,
...
function handlePages(page) {
  ...
  ImmediateInventory.getAllPage(page, currentSize.value, 7)
    .then((res) => {
      const scrollContainer = tableContainer.value.querySelector(
      '.el-scrollbar__wrap'
    )
...
function deleteSelectedRows() {
  ...
  if (list.length == 0) {
    ElMessageBox.alert('请选中至少一行删除', '提示', {
      ...
      .then(() => {
        ImmediateInventory.deleteData(list, 7)
          .then((res) => {
            console.log("删除产能成功");
            ElMessage({
              type: "success",
