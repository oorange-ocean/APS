    ...
    <div class="container">
        ...
        <div class="main" ref="tableContainer">
            <div class="common" ref="commonPlan">
                <common-plan class="plan" :columnNames="ImmediateInventory.purchaseOrder.column"
                    :viewColumn="ImmediateInventory.purchaseOrder.viewColumn" :currentViewId="currentViewId"
                    :currentViewName="currentViewName" :apiUrl="'/interface/searchLike'" :currentTableId="6"
                    :currentOrder="currentOrder" @lookView="lookView" @searchView="searchView"
                    @getCurrentOption="getCurrentOption" />
            </div>
            <el-table ref="myTable" :data="ImmediateInventory.purchaseOrder.data" border
            ...
            <div class="bottom" :style="{
                ...
                <Pagination :total="ImmediateInventory.purchaseOrder.pages" @change-page="handlePages"
                    @update-size="handleSizeChange" :totalRows="ImmediateInventory.purchaseOrder.total"
...
import { columnConfig } from '@/columnConfig/ERP/purchaseOrder'
...
const dynamicColumns = computed(() => {
    if (currentViewId.value === '-1') {
        ...
        return columnConfig
    } else if (!ImmediateInventory.purchaseOrder.viewColumn) {
        return []
    }

    return ImmediateInventory.purchaseOrder.viewColumn
        .map(viewCol => {
...
function lookView(viewId, viewName) {
    ...
    ImmediateInventory.getPageFiltrate(
        ...
        .then((res) => {
            ...
            viewColumn = ImmediateInventory.purchaseOrder.viewColumn
            // console.log(viewColumn,'viewColumn')
            if (viewId == '-1') {
                // 当 viewId 为 -1 时，使用所有列
                ImmediateInventory.purchaseOrder.viewColumn = columnConfig.map(col => ({
                    voColName: col.prop,
...
function addRow() {
    if (addAble) {
        ...
        ImmediateInventory.purchaseOrder.data.unshift({ ...newRow })
        addAble = false
    } else {
        return
    }
...
function deleteSelectedRows() {
    ...
    if (list.length == 0) {
        ...
        ElMessageBox.confirm('确定删除这些工序吗？', '提示', {
            ...
            .then(() => {
                ImmediateInventory.deleteData(list, 6)
                    .then((res) => {
                        console.log('删除产能成功')
                        ElMessage({
                            type: 'success',
...
function refresh() {
    ...
    ImmediateInventory.getViews(tableId.value).then((res) => {
        currentViewId.value = ImmediateInventory.purchaseOrder.defaultViewId
        currentViewName.value = ImmediateInventory.purchaseOrder.defaultViewName
        // 获取所有的列
        ImmediateInventory.getCols(tableId.value).then((res) => {
            // 获取到列名和视图列后再赋值给column和viewColumn
            column = ImmediateInventory.purchaseOrder.column
            viewColumn = ImmediateInventory.purchaseOrder.viewColumn
            // 如果是“全部”就给plan赋值
            if (currentViewId.value == -1) {
