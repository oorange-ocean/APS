<template>
  ...
  <div class="container">
    ...
    <div class="main" ref="tableContainer">
      <el-table
        :data="ImmediateInventory.purchaseRequest.data"
        border
      ...
      >
        <el-table-column
          ...
          class="one"
        />
        <el-table-column prop="materialId" label="物料编码" width="150">
        ...
        <el-table-column prop="baseUnitQty" label="批准数量" width="150">
          <template #default="{ row }">
            <template v-if="row.editable">
      ...
      <div class="bottom" :style="{ width: userMenu.isCollapse ? 'calc(100vw - 50px)' : 'calc(100vw - 250px)' }">
        <Pagination
          :total="ImmediateInventory.purchaseRequest.pages"
          @change-page="handlePages"
          @update-size="handleSizeChange"
          :totalRows="ImmediateInventory.purchaseRequest.total"
        />
      </div>
    </div>
...
function handleSizeChange(newSize) {
  ...
  ImmediateInventory.getAllPage(currentPage.value, currentSize.value, 6)
    .then((res) => {
...
function addRow() {
  if (addAble) {
    ...
    ImmediateInventory.purchaseRequest.data.unshift({ ...newRow });
    addAble = false;
  } else {
    return;
  }
...
function handlePages(page) {
  ...
  ImmediateInventory.getAllPage(page, currentSize.value, 6)
    .then((res) => {
      const scrollContainer = tableContainer.value.querySelector(
      '.el-scrollbar__wrap'
    )
