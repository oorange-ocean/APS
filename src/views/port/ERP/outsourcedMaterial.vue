<template>
  <div class="container">
    ...
    <div class="main" ref="tableContainer">
      <el-table
        :data="ImmediateInventory.outsourcedMaterial.data"
        border
      ...
      >
        <el-table-column
          ...
          class="one"
        />
        <el-table-column prop="materialId" label="产品编码" width="110">
        ...
        <el-table-column prop="materialName" label="产品名称" width="280">
          <template #default="{ row }">
            <template v-if="row.editable">
              <el-input
        ...
        <el-table-column prop="subReqBillNo" label="委外订单编号" width="130">
          <template #default="{ row }">
            <template v-if="row.editable">
              <el-input
                v-model="row.subReqBillNo"
                @keyup.enter="saveRow(row)"
              />
            </template>
            <template v-else>
        ...
        <el-table-column prop="materialId2" label="子项物料编码" width="130">
          <template #default="{ row }">
            <template v-if="row.editable">
              <el-input v-model="row.materialId2" @keyup.enter="saveRow(row)" />
            </template>
            <template v-else>
              {{ row.materialId2 }}
            </template>
        ...
        <el-table-column prop="materialName2" label="子项物料名称" width="230">
        </el-table-column>
        <el-table-column prop="materialType" label="子项类型" width="85">
          <template #default="{ row }">
            <template v-if="row.editable">
              <el-input
        ...
        <el-table-column prop="mustQty" label="应发数量" width="85">
          <template #default="{ row }">
            <template v-if="row.editable">
        ...
        <el-table-column prop="pickedQty" label="已领数量" width="85">
          <template #default="{ row }">
            <template v-if="row.editable">
              <el-input v-model="row.pickedQty" @keyup.enter="saveRow(row)" />
            </template>
            <template v-else>
        ...
        <el-table-column prop="goodReturnQty" label="良品退料数量">
          <template #default="{ row }">
            <template v-if="row.editable">
              <el-input
                v-model="row.goodReturnQty"
                @keyup.enter="saveRow(row)"
              />
            </template>
            <template v-else>
        ...
        <el-table-column prop="processDefectReturnQty" label="作业不良退料数量">
          <template #default="{ row }">
            <template v-if="row.editable">
              <el-input
                v-model="row.processDefectReturnQty"
                @keyup.enter="saveRow(row)"
              />
            </template>
            <template v-else>
      ...
      >
        <Pagination
          :total="ImmediateInventory.outsourcedMaterial.pages"
          @change-page="handlePages"
          @update-size="handleSizeChange"
          :totalRows="ImmediateInventory.outsourcedMaterial.total"
        />
      </div>
    </div>
  </div>
...
function handleSizeChange(newSize) {
  ...
  ImmediateInventory.getAllPage(currentPage.value, currentSize.value, 2)
    .then((res) => {
...
function addRow() {
  if (addAble) {
    ...
    ImmediateInventory.outsourcedMaterial.data.unshift({ ...newRow })
    addAble = false
  } else {
    return
  }
    ...
    ImmediateInventory.update(
      {
        ...
        subReqBillNo: row.subReqBillNo,
        materialId2: row.materialId2,
        materialName2: row.materialName2,
        materialType: row.materialType,
        mustQty: row.mustQty,
        pickedQty: row.pickedQty,
        goodReturnQty: row.goodReturnQty,
        processDefectReturnQty: row.processDefectReturnQty
      },
      2
    )
      .then((res) => {
...
function handlePages(page) {
  ...
  ImmediateInventory.getAllPage(page, currentSize.value, 2)
    .then((res) => {
      const scrollContainer = tableContainer.value.querySelector(
        '.el-scrollbar__wrap'
      )
...
function deleteSelectedRows() {
  ...
  if (list.length == 0) {
    ...
    ElMessageBox.confirm('确定删除这些工序吗？', '提示', {
      ...
      .then(() => {
        ImmediateInventory.deleteData(list, 2)
          .then((res) => {
            console.log('删除产能成功')
            ElMessage({
              type: 'success',
