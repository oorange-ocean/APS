    ...
    <div class="container">
        ...
        <div class="main" ref="tableContainer">
            <div class="common" ref="commonPlan">
                <common-plan class="plan" :columnNames="ImmediateInventory.productionOrder.column"
                    :viewColumn="ImmediateInventory.productionOrder.viewColumn" :currentViewId="currentViewId"
                    :currentViewName="currentViewName" :apiUrl="'/interface/searchLike'" :currentTableId="4"
                    :currentOrder="currentOrder" @lookView="lookView" @searchView="searchView"
                    @getCurrentOption="getCurrentOption" />
            </div>
            <el-table ref="myTable" :data="ImmediateInventory.productionOrder.data" border
            ...
            <div class="bottom" :style="{
                ...
                <Pagination :total="ImmediateInventory.productionOrder.pages" @change-page="handlePages"
                    @update-size="handleSizeChange" :totalRows="ImmediateInventory.productionOrder.total"
...
import { columnConfig } from '../../../columnConfig/ERP/productionOrder'
...
const dynamicColumns = computed(() => {
    if (currentViewId.value === '-1') {
        ...
        return columnConfig
    } else if (!ImmediateInventory.productionOrder.viewColumn) {
        return []
    }

    return ImmediateInventory.productionOrder.viewColumn
        .map(viewCol => {
...
function lookView(viewId, viewName) {
    ...
    ImmediateInventory.getPageFiltrate(
        ...
        .then((res) => {
            ...
            viewColumn = ImmediateInventory.productionOrder.viewColumn
            // console.log(viewColumn,'viewColumn')
            if (viewId == '-1') {
                // 当 viewId 为 -1 时，使用所有列
                ImmediateInventory.productionOrder.viewColumn = columnConfig.map(col => ({
                    voColName: col.prop,
...
function addRow() {
    if (addAble) {
        ...
        ImmediateInventory.productionOrder.data.unshift({ ...newRow })
        addAble = false
    } else {
        return
    }
...
function saveRow(row) {
    ...
    if (row.id) {
        ImmediateInventory.update(
            {
                id: row.id,
                plannedCompletionTime: row.plannedCompletionTime,
                ...
                stockInQuaAuxQty: row.stockInQuaAuxQty,
                qty: row.qty,
                formName: row.formName,
                dzmaterialId: row.dzmaterialId
            },
            4
        )
            .then((res) => {
...
function deleteSelectedRows() {
    ...
    if (list.length == 0) {
        ...
        ElMessageBox.confirm('确定删除这些工序吗？', '提示', {
            ...
            .then(() => {
                ImmediateInventory.deleteData(list, 4)
                    .then((res) => {
                        console.log('删除产能成功')
                        ElMessage({
                            type: 'success',
...
function refresh() {
    ...
    ImmediateInventory.getViews(tableId.value).then((res) => {
        currentViewId.value = ImmediateInventory.productionOrder.defaultViewId
        currentViewName.value = ImmediateInventory.productionOrder.defaultViewName
        // 获取所有的列
        ImmediateInventory.getCols(tableId.value).then((res) => {
            // 获取到列名和视图列后再赋值给column和viewColumn
            column = ImmediateInventory.productionOrder.column
            viewColumn = ImmediateInventory.productionOrder.viewColumn
            // 如果是“全部”就给plan赋值
            if (currentViewId.value == -1) {
