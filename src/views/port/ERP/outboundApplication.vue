    ...
    <div class="container">
        ...
        <div class="main" ref="tableContainer">
            <div class="common" ref="commonPlan">
                <common-plan class="plan" :columnNames="ImmediateInventory.outboundApplication.column"
                    :viewColumn="ImmediateInventory.outboundApplication.viewColumn" :currentViewId="currentViewId"
                    :currentViewName="currentViewName" :apiUrl="'/interface/searchLike'" :currentTableId="11"
                    :currentOrder="currentOrder" @lookView="lookView" @searchView="searchView"
                    @getCurrentOption="getCurrentOption" />
            </div>
            <el-table ref="myTable" :data="ImmediateInventory.outboundApplication.data" border
            ...
            <div class="bottom" :style="{
                ...
                <Pagination :total="ImmediateInventory.outboundApplication.pages" @change-page="handlePages"
                    @update-size="handleSizeChange" :totalRows="ImmediateInventory.outboundApplication.total"
...
import { columnConfig } from '../../../columnConfig/ERP/outboundApplication'
...
const dynamicColumns = computed(() => {
    if (currentViewId.value === '-1') {
        ...
        return columnConfig
    } else if (!ImmediateInventory.outboundApplication.viewColumn) {
        return []
    }

    return ImmediateInventory.outboundApplication.viewColumn
        .map(viewCol => {
...
function lookView(viewId, viewName) {
    ...
    ImmediateInventory.getPageFiltrate(
        ...
        .then((res) => {
            ...
            viewColumn = ImmediateInventory.outboundApplication.viewColumn
            // console.log(viewColumn,'viewColumn')
            if (viewId == '-1') {
                // 当 viewId 为 -1 时，使用所有列
                ImmediateInventory.outboundApplication.viewColumn = columnConfig.map(col => ({
                    voColName: col.prop,
...
function deleteSelectedRows() {
    ...
    if (list.length == 0) {
        ...
        ElMessageBox.confirm('确定删除这些工序吗？', '提示', {
            ...
            .then(() => {
                ImmediateInventory.deleteData(list, 11)
                    .then((res) => {
                        console.log('删除产能成功')
                        ElMessage({
                            type: 'success',
...
function refresh() {
    ...
    ImmediateInventory.getViews(tableId.value).then((res) => {
        ...
        ImmediateInventory.getCols(tableId.value).then((res) => {
            ...
            column = ImmediateInventory.outboundApplication.column
            viewColumn = ImmediateInventory.outboundApplication.viewColumn
            // 如果是“全部”就给plan赋值
            if (currentViewId.value == -1) {
