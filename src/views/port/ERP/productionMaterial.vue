    ...
    <div class="container">
        ...
        <div class="main" ref="tableContainer">
            <div class="common" ref="commonPlan">
                <common-plan class="plan" :columnNames="ImmediateInventory.productionMaterial.column"
                    :viewColumn="ImmediateInventory.productionMaterial.viewColumn" :currentViewId="currentViewId"
                    :currentViewName="currentViewName" :apiUrl="'/interface/searchLike'" :currentTableId="2"
                    :currentOrder="currentOrder" @lookView="lookView" @searchView="searchView"
                    @getCurrentOption="getCurrentOption" />
            </div>
            <el-table ref="myTable" :data="ImmediateInventory.productionMaterial.data" border
            ...
            <div class="bottom" :style="{
                ...
                <Pagination :total="ImmediateInventory.productionMaterial.pages" @change-page="handlePages"
                    @update-size="handleSizeChange" :totalRows="ImmediateInventory.productionMaterial.total"
...
import { columnConfig } from '@/columnConfig/ERP/productionMaterial'
...
const dynamicColumns = computed(() => {
    if (currentViewId.value === '-1') {
        ...
        return columnConfig
    } else if (!ImmediateInventory.productionMaterial.viewColumn) {
        return []
    }

    return ImmediateInventory.productionMaterial.viewColumn
        .map(viewCol => {
...
function lookView(viewId, viewName) {
    ...
    ImmediateInventory.getPageFiltrate(
        ...
        .then((res) => {
            ...
            if (viewId == '-1') {
                // 当 viewId 为 -1 时，使用所有列
                ImmediateInventory.productionMaterial.viewColumn = columnConfig.map(col => ({
                    voColName: col.prop,
...
function addRow() {
    if (addAble) {
        ...
        ImmediateInventory.productionMaterial.data.unshift({ ...newRow })
        addAble = false
    } else {
        return
    }
