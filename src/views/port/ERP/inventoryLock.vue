    ...
    <div class="container">
        ...
        <div class="main" ref="tableContainer">
            <div class="common" ref="commonPlan">
                <common-plan class="plan" :columnNames="ImmediateInventory.inventoryLock.column"
                    :viewColumn="ImmediateInventory.inventoryLock.viewColumn" :currentViewId="currentViewId"
                    :currentViewName="currentViewName" :apiUrl="'/interface/searchLike'" :currentTableId="9"
                    :currentOrder="currentOrder" @lookView="lookView" @searchView="searchView"
                    @getCurrentOption="getCurrentOption" />
            </div>
            <el-table ref="myTable" :data="ImmediateInventory.inventoryLock.data" border
            ...
            <div class="bottom" :style="{
                ...
                <Pagination :total="ImmediateInventory.inventoryLock.pages" @change-page="handlePages"
                    @update-size="handleSizeChange" :totalRows="ImmediateInventory.inventoryLock.total"
...
import { columnConfig } from '@/columnConfig/ERP/inventoryLock'
...
const dynamicColumns = computed(() => {
    if (currentViewId.value === '-1') {
        ...
        return columnConfig
    } else if (!ImmediateInventory.inventoryLock.viewColumn) {
        return []
    }

    return ImmediateInventory.inventoryLock.viewColumn
        .map(viewCol => {
...
function lookView(viewId, viewName) {
    ...
    ImmediateInventory.getPageFiltrate(
        ...
        .then((res) => {
            ...
            if (viewId == '-1') {
                // 当 viewId 为 -1 时，使用所有列
                ImmediateInventory.inventoryLock.viewColumn = columnConfig.map(col => ({
                    voColName: col.prop,
