<template>
    ...
    <div class="container">
      ...
      <div class="main" ref="tableContainer">
        <el-table
          :data="ImmediateInventory.installationBoard.data"
          border
      ...
      <div class="bottom">
        <Pagination
          :total="ImmediateInventory.installationBoard.pages"
          @change-page="handlePages"
          @update-size="handleSizeChange"
          :totalRows="ImmediateInventory.installationBoard.total"
        />
      </div>
    </div>
  </template>
  ...
  function handleSizeChange(newSize) {
    ...
    ImmediateInventory.getAllPage(currentPage.value, currentSize.value, 24)
      .then((res) => {
  ...
  function addRow() {
    ...
    if (scrollContainer) {
      ...
      ImmediateInventory.installationBoard.data.unshift({ ...newRow });
      addAble = false;
    } else {
      return;
    }
  ...
  function handlePages(page) {
    ...
    ImmediateInventory.getAllPage(page, currentSize.value, 24)
      .then((res) => {
        const scrollContainer = tableContainer.value.querySelector(
      '.el-scrollbar__wrap'
    )
