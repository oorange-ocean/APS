<template>
    ...
    <div class="container">
      ...
      <div class="main" ref="tableContainer">
        <el-table
          :data="ImmediateInventory.tfminisPackagingTest.data"
          border
        ...
        >
          ...
          <el-table-column prop="burnQualifiedCount" label="包装终检合格数" width="100">
            <template #default="{ row }">
              <template v-if="row.editable">
          ...
          <el-table-column prop="unBurnQualifiedCount" label="包装终检不合格数" width="90">
            <template #default="{ row }">
              <template v-if="row.editable">
      ...
      <div class="bottom">
        <Pagination
          :total="ImmediateInventory.tfminisPackagingTest.pages"
          @change-page="handlePages"
          @update-size="handleSizeChange"
          :totalRows="ImmediateInventory.tfminisPackagingTest.total"
        />
      </div>
    </div>
  </template>
  ...
  function handleSizeChange(newSize) {
    ...
    ImmediateInventory.getAllPage(currentPage.value, currentSize.value, 31)
      .then((res) => {
  ...
  function addRow() {
    ...
    if (scrollContainer) {
      ...
      ImmediateInventory.tfminisPackagingTest.data.unshift({ ...newRow });
      addAble = false;
    } else {
      return;
    }
  ...
  function handlePages(page) {
    ...
    ImmediateInventory.getAllPage(page, currentSize.value, 31)
      .then((res) => {
        const scrollContainer = tableContainer.value.querySelector(
      '.el-scrollbar__wrap'
    )
