    ...
    <div class="container">
        ...
        <div class="main" ref="tableContainer">
            <div class="common" ref="commonPlan">
                <common-plan class="plan" :columnNames="ImmediateInventory.mesUnfinished.column"
                    :viewColumn="ImmediateInventory.mesUnfinished.viewColumn" :currentViewId="currentViewId"
                    :currentViewName="currentViewName" :apiUrl="'/interface/searchLike'" :currentTableId="33"
                    :currentOrder="currentOrder" @lookView="lookView" @searchView="searchView"
                    @getCurrentOption="getCurrentOption" />
            </div>
            <el-table ref="myTable" :data="ImmediateInventory.mesUnfinished.data" border
            ...
            <div class="bottom" :style="{
                ...
                <Pagination :total="ImmediateInventory.mesUnfinished.pages" @change-page="handlePages"
                    @update-size="handleSizeChange" :totalRows="ImmediateInventory.mesUnfinished.total"
...
import { columnConfig } from '@/columnConfig/MES/mesUnfinished'
...
const dynamicColumns = computed(() => {
    if (currentViewId.value === '-1') {
        ...
        return columnConfig
    } else if (!ImmediateInventory.mesUnfinished.viewColumn) {
        return []
    }

    return ImmediateInventory.mesUnfinished.viewColumn
        .map(viewCol => {
...
function lookView(viewId, viewName) {
    ...
    ImmediateInventory.getPageFiltrate(
        ...
        .then((res) => {
            ...
            if (viewId == '-1') {
                // 当 viewId 为 -1 时，使用所有列
                ImmediateInventory.mesUnfinished.viewColumn = columnConfig.map(col => ({
                    voColName: col.prop,
...
function refresh() {
    ...
    ImmediateInventory.getViews(tableId.value).then((res) => {
        ...
        currentViewName.value = ImmediateInventory.mesUnfinished.defaultViewName
        // 获取所有的列
