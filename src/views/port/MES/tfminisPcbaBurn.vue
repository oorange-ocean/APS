<template>
  <div class="container">
    ...
    <div class="main" ref="tableContainer">
      <el-table
        :data="ImmediateInventory.tfminsPcbaBurn.data"
        border
      ...
      <div class="bottom" :style="{ width: userMenu.isCollapse ? 'calc(100vw - 50px)' : 'calc(100vw - 250px)' }">
        <Pagination
          :total="ImmediateInventory.tfminsPcbaBurn.pages"
          @change-page="handlePages"
          @update-size="handleSizeChange"
          :totalRows="ImmediateInventory.tfminsPcbaBurn.total"
        />
      </div>
    </div>
...
function handleSizeChange(newSize) {
  ...
  ImmediateInventory.getAllPage(currentPage.value, currentSize.value, 21)
    .then((res) => {
...
function addRow() {
  if (addAble) {
    ...
    ImmediateInventory.tfminsPcbaBurn.data.unshift({ ...newRow })
    addAble = false
  } else {
    return
  }
...
function handlePages(page) {
  ...
  ImmediateInventory.getAllPage(page, currentSize.value, 21)
    .then((res) => {
      const scrollContainer = tableContainer.value.querySelector(
        '.el-scrollbar__wrap'
      )
...
function deleteSelectedRows() {
  ...
  if (list.length == 0) {
    ...
    ElMessageBox.confirm('确定删除这些工序吗？', '提示', {
      ...
      .then(() => {
        ImmediateInventory.deleteData(list, 21)
          .then((res) => {
            console.log('删除产能成功')
            ElMessage({
              type: 'success',
