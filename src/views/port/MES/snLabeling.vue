<template>
    ...
    <div class="container">
      ...
      <div class="main" ref="tableContainer">
        <el-table
          :data="ImmediateInventory.snLabeling.data"
          border
      ...
      <div class="bottom">
        <Pagination
          :total="ImmediateInventory.snLabeling.pages"
          @change-page="handlePages"
          @update-size="handleSizeChange"
          :totalRows="ImmediateInventory.snLabeling.total"
        />
      </div>
    </div>
  </template>
  ...
  function handleSizeChange(newSize) {
    ...
    ImmediateInventory.getAllPage(currentPage.value, currentSize.value, 26)
      .then((res) => {
  ...
  function addRow() {
    ...
    if (scrollContainer) {
      ...
      ImmediateInventory.snLabeling.data.unshift({ ...newRow });
      addAble = false;
    } else {
      return;
    }
  ...
  function handlePages(page) {
    ...
    ImmediateInventory.getAllPage(page, currentSize.value, 26)
      .then((res) => {
        const scrollContainer = tableContainer.value.querySelector(
      '.el-scrollbar__wrap'
    )
