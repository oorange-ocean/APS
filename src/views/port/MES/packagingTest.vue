<template>
  ...
  function handleSizeChange(newSize) {
    ...
    ImmediateInventory.getAllPage(currentPage.value, currentSize.value, 30)
      .then((res) => {
  ...
  function addRow() {
    ...
    if (scrollContainer) {
      ...
      ImmediateInventory.packagingTest.data.unshift({ ...newRow });
      addAble = false;
    } else {
      return;
    }
  ...
  function handlePages(page) {
    ...
    ImmediateInventory.getAllPage(page, currentSize.value, 30)
      .then((res) => {
        const scrollContainer = tableContainer.value.querySelector(
      '.el-scrollbar__wrap'
    )
