<template>
  ...
  <div class="container">
    ...
    <div class="main" ref="tableContainer">
      <el-table
        :data="ImmediateInventory.tfminisPcbaVersion.data"
        border
      ...
      >
        ...
        <el-table-column prop="burnQualifiedCount" label="分板合格数" width="100">
          <template #default="{ row }">
            <template v-if="row.editable">
        ...
        <el-table-column prop="unburnQualifiedCount" label="分版不合格数" width="90">
          <template #default="{ row }">
            <template v-if="row.editable">
        ...
        <el-table-column prop="burnFixtureNumber" label="分板治具编号" width="120">
          <template #default="{ row }">
            <template v-if="row.editable">
              <el-input
        ...
        <el-table-column prop="totalNumber" label="分板总数量" width="120">
          <template #default="{ row }">
            <template v-if="row.editable">
              <el-input
    ...
    <div class="bottom">
      <Pagination
        :total="ImmediateInventory.tfminisPcbaVersion.pages"
        @change-page="handlePages"
        @update-size="handleSizeChange"
        :totalRows="ImmediateInventory.tfminisPcbaVersion.total"
      />
    </div>
  </div>
</template>
...
function handleSizeChange(newSize) {
  ...
  ImmediateInventory.getAllPage(currentPage.value, currentSize.value, 23)
    .then((res) => {
...
function addRow() {
  ...
  if (scrollContainer) {
    ...
    ImmediateInventory.tfminisPcbaVersion.data.unshift({ ...newRow });
    addAble = false;
  } else {
    return;
  }
  ...
  if (row.id) {
    ImmediateInventory.update
      ({
        ...
        "burnQualifiedCount": row.burnInCompletionQuantity,
...
function handlePages(page) {
  ...
  ImmediateInventory.getAllPage(page, currentSize.value, 23)
    .then((res) => {
      const scrollContainer = tableContainer.value.querySelector(
    '.el-scrollbar__wrap'
  )
